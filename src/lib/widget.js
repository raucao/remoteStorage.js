define(['./session'], function (session, sync) {
  ...
  function displayWidgetState(state) {
    var spin = (state == 'unknown' || state == 'connecting' || layout == 'busy'),
  ...
  function display(setConnectElement, setLocale) {
    ...
    platform.on(connectElement, 'hover', handleWidgetHover);
    platform.on('remotestorage-useraddres', 'type', handleWidgetType);
