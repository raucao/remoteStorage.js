...
function windowLoadHandler() {
    require(['../../src/remoteStorage'], function(remoteStorage) {
      ...
      remoteStorageClient = remoteStorage.createClient('tasks/todolist');
...
function loadTodos() {
    todos = remoteStorageClient.getAll();
}
...
function removeTodoById( id ) {
    ...
    for ( i=0; i < todos.length; i++ ) {
        if ( todos[i].id === id ) {
            remoteStorageClient.removeItem(todos[i].id);
            todos.splice( i, 1 );
        }
    }
}
...
function saveTodos() {
    var i;
    for ( i=0; i < todos.length; i++ ) {
      remoteStorageClient.setItem(todos[i].id, todos[i]);
    }
}
